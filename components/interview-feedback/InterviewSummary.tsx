...
const InterviewSummary: React.FC<InterviewSummaryProps> = ({
  ...
  return (
    <Card className="mb-6 overflow-hidden border-border bg-card shadow-md">
      <CardHeader className="border-b border-border bg-background p-6 text-foreground">
      ...
      <CardContent className="p-6">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground">{summary.overall_analysis}</p>
        </div>
              ...
              {summary?.notable_strengths?.length > 0
                ? summary.notable_strengths.map((strength, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <span className="mt-1 text-orange-300">
                        <svg
