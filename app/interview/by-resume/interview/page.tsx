        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-background border border-border rounded-lg p-4">
            ...
            <ol className="list-decimal pl-5 space-y-2 text-sm text-muted-foreground">
              ...
              <li>Have your resume readily available</li>
...
const InterviewPageContent = () => {
  ...
  useEffect(() => {
    ...
    console.log("Retrieved resume data from sessionStorage:", data);
    if (data) {
  ...
  }, [level, duration, resumeData, router]);
  ...
  const setupInterview = async () => {
    setLoading({ state: true, message: "Generating questions..." });
    try {
      // Adjust number of questions based on duration
      const durationMinutes = Math.floor(timeRemaining / 60);
  ...
  const startInterview = async () => {
    ...
    await vapi.start(VAPI_ASSISTANT_ID, {
      ...
      variableValues: {
        ...
        job_title: "Resume Based Interview",
  ...
  async function endInterview() {
    ...
    console.log("=== ENDING RESUME INTERVIEW ===");
    console.log("Messages count:", messages.length);
    console.log("Questions count:", interviewQuestions.length);
    console.log("Interview ID:", interviewId);
    console.log("Has resume data:", !!resumeData);

    const useResumeFallback = !messages || messages.length < 2;

    if (!useResumeFallback && messages.length < 2) {
      toast.warning("Short interview detected", {
        description: "Using resume-based assessment",
      });
    }

    setLoading({
      state: true,
      message: useResumeFallback
        ? "Generating resume-based assessment..."
        : "Generating feedback for you...",
    });
    ...
    try {
      const feedbackPayload = {
        ...
        job: {
          ...
          level: level,
          type: "Full-time",
          requirements: (resumeData as any)?.skills || [],
          responsibilities: [],
          company: "Company",
        },
        ...
        resumeData: resumeData,
        useResumeFallback: useResumeFallback,
      };
      ...
      console.log("Sending feedback request with payload:", {
        ...
        using_resume_fallback: useResumeFallback,
      });
      ...
      if (result.data.success) {
        ...
        toast.success(
          useResumeFallback
            ? "Resume-based assessment generated!"
      ...
      if (resumeData && !useResumeFallback) {
        console.log("Attempting fallback to resume-based assessment...");
        setLoading({
          state: true,
          message: "Generating feedback based on your resume...",
        });
        ...
        try {
          ...
          if (fallbackResult.data.success) {
            ...
            const finalInterviewId =
              fallbackResult.data.interviewId || interviewId;
            toast.success("Feedback generated based on your resume!");
            router.push(`/feedback/${finalInterviewId}`);
            return;
          }
      ...
      interval = setInterval(() => {
        setTimeRemaining((prev) => {
          if (prev <= 1) {
            ...
            vapi.say(
              "That's the end of your interview. Thank you for your time. We'll get back to you with feedback soon.",
              true
            );
